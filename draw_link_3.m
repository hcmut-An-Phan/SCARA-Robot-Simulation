function draw_link_3(handles,varargin)
...
th = linspace(0,2*pi, nNodes); % draw full 2pi
% transform polar cylindrical coordinates to castersian coordiantes
[x,y] = pol2cart(th,r);
% translate to origin
x = x + origin(1);
y = y + origin(2);
z = linspace(origin(3) + theta,origin(3)+theta+length,nCS)';
%  Replication
x = repmat(x,nCS,1);
y = repmat(y,nCS,1);
z = repmat(z,1,nNodes);
...
z_lid = repmat([origin(3) + theta;origin(3)+theta+length],1,nNodes);
